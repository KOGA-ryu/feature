...
pub fn show(ui: &mut egui::Ui, app: &mut FeatureLabApp) {
    ...
    let rows = app
        .visible_browser_entries()
        .into_iter()
        .cloned()
        .collect::<Vec<_>>();
    let mut category_counts = BTreeMap::new();
    let visible_counts = app.visible_feature_counts();
    for row in &rows {
        *category_counts
            .entry(category_of(&row.manifest.id))
            .or_insert(0usize) += 1;
    }
    let mut current_category = String::new();

    egui::ScrollArea::vertical()
        .id_salt("feature_list_scroll")
        .show(ui, |ui| {
            summary_card(
                ui,
                rows.len(),
                visible_counts.ui,
                visible_counts.logic,
                visible_counts.sim,
                visible_counts.workflow,
                app.kind_filter().label(),
                app.browser.query().trim(),
            );
            ui.add_space(10.0);

            for row in rows {
                let row_category = category_of(&row.manifest.id);
                ...
                let selected = app.selected_feature_id() == Some(row.manifest.id.as_str());
                ...
                let response = egui::Frame::group(ui.style())
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ui.strong(&row.manifest.name);
                            let category_color = category_color(&row.manifest.id);
                            egui::Frame::new()
                                ...
                                .show(ui, |ui| {
                                    ui.small(
                                        egui::RichText::new(kind_label(&row.manifest.id))
                            ...
                            ui.with_layout(
                                ...
                                |ui| {
                                    let badge = egui::RichText::new(
                                        row.manifest.status.to_string().to_uppercase(),
                                    )
                                    .small()
                                    .color(status_color(&row.manifest.status.to_string()));
                                    ui.label(badge);
                                },
                            );
                        });
                        ui.small(egui::RichText::new(&row.manifest.id).monospace());
                        ui.add_space(2.0);
                        ui.label(egui::RichText::new(&row.manifest.summary).small());
                    })
                    .response;

                if response.clicked() {
                    app.select_feature(row.manifest.id.clone());
                    app.push_log(format!("Selected feature {}", row.manifest.id));
                }
...
fn category_of(feature_id: &str) -> String {
    feature_id
        .split('.')
        .next()
